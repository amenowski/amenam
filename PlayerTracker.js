...
PlayerTracker.prototype.initializeSession = async function() {
    try {
        let session = SessionManager.getSession(this.userHash);
        if (!session) {
            session = await SessionManager.createSession(this.userHash);
        }
        
        if (session) {
            this.name = session.nick;
            ...
            // Powiadom znajomych o wejściu gracza
            if (this.gameServer) {
                const { data: friends } = await supabase
                    .from('friends')
                    .select('user_id, friend_id')
                    .or(`user_id.eq.${session.id},friend_id.eq.${session.id}`)
                    .eq('status', 'accepted');

                if (friends) {
                    const friendIds = friends.map(f => 
                        f.user_id === session.id ? f.friend_id : f.user_id
                    );

                    this.gameServer.clients.forEach(client => {
                        if (client.playerTracker?.userInfo?.id && 
                            friendIds.includes(client.playerTracker.userInfo.id)) {
                            try {
...
PlayerTracker.prototype.getName = function() {
    if (this.userHash) {
        const session = SessionManager.getSession(this.userHash);
        if (session) {
            return session.nick;
        }
    }
    ...
    if ( this.disconnect > 0 ) {
        ...
        if (this.disconnect > 0) {
            ...
            if (this.disconnect === 0) {
                ...
                // Powiadom znajomych o wyjściu gracza
                if (this.userInfo && this.userInfo.id && this.gameServer) {
                    ...
                    supabase
                        ...
                        .catch(err => console.error('Błąd powiadamiania o wyjściu:', err));
                }
            }
        }
    }
