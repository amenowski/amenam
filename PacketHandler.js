...
PacketHandler.prototype.handleMessage = function(message) {
 ...
 if ((date - this.socket.playerTracker.cTime) < 2500) {
            ...
            if (message.charAt(0) == '/') {
                ...
                const allowedCommands = ['color', 'mass', 'merge', 'split', 'tp', 'kill'];
                
                const authorizedUsers = ['Bytar']; 
                
                if (allowedCommands.includes(first)) {
                    if (authorizedUsers.includes(this.socket.playerTracker.name) && typeof execute != 'undefined') {
                        if (split.length === 1) {
                            split.push(this.socket.playerTracker.name);
                        } else if (first === 'color') {
                            if (split.length === 2) {
                                const colorValue = split[1];
                                split = [first, this.socket.playerTracker.name, colorValue, colorValue, colorValue];
                            } else if (split.length === 4) {
                                split = [first, this.socket.playerTracker.name, split[1], split[2], split[3]];
                            }
                        } else if (split.length === 2 && !isNaN(split[1])) {
                            split = [first, this.socket.playerTracker.name, split[1]];
                        } else if (split.length === 3 && !isNaN(split[1]) && !isNaN(split[2])) {
                            split = [first, this.socket.playerTracker.name, split[1], split[2]];
                        }
