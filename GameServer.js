...
async function getPlayerColorFromDatabase(player) {
    try {
        ...
        if (data.color[0] === 0 && data.color[1] === null && data.color[2] === null) {
            return {
                r: 0,
                g: 0,
                b: 0
...
// here
    ...
    this.socketServer = new WebSocket.Server({server: this.httpServer, perMessageDeflate: false});
        ...
        if (this.clients.length >= this.config.serverMaxConnections) { // Server full
            ...
            return;
                ...
                if (this.config.serverMaxConnPerIp) {
                    ...
                    if (cons > 30) { // Możesz dostosować tę wartość
                        ws.close();
                        return;
                    }
                }
