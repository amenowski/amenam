...
async function getPlayerColorFromDatabase(player) {
  try {
    ...
    console.log(`[COLOR] Pobieranie koloru dla ${playerName}`);
    ...
    if (data.color.some((value) => value === null)) {
      return null;
    }

    const color = {
      r: parseInt(data.color[0], 10),
      g: parseInt(data.color[1], 10),
      b: parseInt(data.color[2], 10),
    };
    ...
    console.log(`[COLOR] Znaleziono kolor dla ${playerName}:`, color);
    return color;
  } catch (error) {
    console.log(`[COLOR] Błąd:`, error.message);
    return null;
  }
}
...
GameServer.prototype.getPlayerColor = async function (player) {
  ...
  if (!playerName) {
    console.log(`[COLOR] Brak nazwy gracza`);
    return this.getRandomColor();
  }

  // Pobierz kolor z bazy
  ...
  // Jeśli nie ma koloru, wygeneruj losowy
  const randomColor = this.getRandomColor();
  console.log(`[COLOR] Wygenerowano losowy kolor dla ${playerName}`);
  return randomColor;
};
...
GameServer.prototype.start = function () {
  ...
  if (this.config.serverAdminPass != "") {
    console.log(
      "* \u001B[33mRcon enabled, passkey set to " +
        this.config.serverAdminPass +
        "\u001B[0m"
    );
    console.log(
      "* \u001B[33mTo use in chat type /rcon " +
        this.config.serverAdminPass +
        " <server command>\u001B[0m"
    );
  }
  ...
  if (this.sqlconfig.host != "") {
    console.log(
      "* \u001B[33mMySQL config loaded Database set to " +
    ...
    if (this.clients.length >= this.config.serverMaxConnections) {
      ...
      console.log(
        "\u001B[33mClient " +
          ...
          ", tried to connect but is banned!\u001B[0m"
      );
      ws.close();
      return;
    }
...
GameServer.prototype.onHttpServerOpen = function () {
  ...
  // Done
  console.log(
    "* \u001B[33mListening on port " + this.config.serverPort + " \u001B[0m"
  );
  console.log(
    "* \u001B[33mCurrent game mode is " + this.gameMode.name + "\u001B[0m"
  );
  ...
  if (this.config.serverBots > 0) {
    ...
    console.log(
      "* \u001B[33mLoaded " + this.config.serverBots + " player bots\u001B[0m"
    );
  }
  if (this.config.serverResetTime > 0) {
    console.log(
      "* \u001B[33mAuto shutdown after " +
        this.config.serverResetTime +
        " hours\u001B[0m"
    );
  }
  ...
  if (this.config.serverVersion == 1)
    console.log(
      "* \u001B[33mProtocol set to new, clients with version 561.20 and up can connect to this server\u001B[0m"
    );
  if (this.config.serverVersion == 0)
    console.log(
      "* \u001B[33mProtocol set to old, clients with version 561.19 and older can connect to this server\u001B[0m"
    );
};
...
GameServer.prototype.getRandomPosition = function () {
  return {
    x:
      Math.floor(
        Math.random() * (this.config.borderRight - this.config.borderLeft)
      ) + this.config.borderLeft,
    y:
      Math.floor(
        Math.random() * (this.config.borderBottom - this.config.borderTop)
      ) + this.config.borderTop,
  };
};
...
GameServer.prototype.getRandomColor = function () {
  const predefinedColors = [
    { r: 7, g: 127, b: 255 }, // #077fff
    { r: 90, g: 7, b: 255 }, // #5a07ff
    { r: 255, g: 7, b: 254 }, // #ff07fe
    { r: 255, g: 165, b: 7 }, // #ffa507
    { r: 255, g: 7, b: 116 }, // #ff0774
    { r: 7, g: 145, b: 255 }, // #0791ff
    { r: 58, g: 255, b: 7 }, // #3aff07
    { r: 255, g: 7, b: 237 }, // #ff07ed
    { r: 7, g: 168, b: 255 }, // #07a8ff
    { r: 255, g: 7, b: 110 }, // #ff076e
    { r: 63, g: 255, b: 7 }, // #3fff07
    { r: 255, g: 7, b: 52 }, // #ff0734
    { r: 7, g: 255, b: 32 }, // #07ff20
    { r: 255, g: 7, b: 162 }, // #ff07a2
    { r: 255, g: 130, b: 7 }, // #ff8207
    { r: 7, g: 255, b: 14 }, // #07ff0e
  ];
...
GameServer.prototype.addNode = function (node) {
  ...
  for (var i = 0; i < this.clients.length; i++) {
    ...
    if (
      "_socket" in client.socket &&
      node.visibleCheck(client.viewBox, client.centerPos)
    ) {
...
GameServer.prototype.mainLoop = function () {
  ...
  if (this.tick >= 50) {
    ...
    if (this.tickMain >= 20) {
      ...
      this.lb_packet = new Packet.UpdateLeaderboard(
        this.leaderboard,
    ...
    this.clients.forEach(function (client) {
      if (client.playerTracker && !client.playerTracker.spectate) players++;
    });
    ...
    if (
      ...
      local - this.startTime > this.config.serverResetTime * 3600000
    ) {
      this.exitserver();
    }
...
GameServer.prototype.updateFood = function () {
  var toSpawn = Math.min(
    this.config.foodSpawnAmount,
...
GameServer.prototype.spawnFood = function () {
  var f = new Entity.Food(
    this.getNextNodeId(),
    null,
    this.getRandomPosition(),
    Math.floor(Math.random() * this.config.foodMaxMass) + this.config.foodMass
  );
...
GameServer.prototype.spawnPlayer = async function (player, pos, mass) {
  if (pos == null) {
  ...
  // Pobierz kolor przed utworzeniem komórki
  const color = await this.getPlayerColor(player);
  player.color = color; // Ustaw kolor na obiekcie gracza
  ...
  if ("_socket" in player.socket) {
    ...
    if (this.config.serverResetTime > 0) {
      var packet = new Packet.BroadCast(
        "Remember, This server auto restarts after " +
          this.config.serverResetTime +
          " hours uptime!"
      );
...
GameServer.prototype.newCellVirused = function (
  client,
  parent,
  angle,
  mass,
...
GameServer.prototype.startStatsServer = function (port) {
  ...
  this.httpServer.listen(
    ...
    function () {
      // Stats server
      console.log(
        "* \u001B[33mLoaded stats server on port " + port + "\u001B[0m"
      );
      setInterval(
...
GameServer.prototype.getStats = function () {
  for (
    var i = 0, humans = 0, bots = 0, players = 0, spectate = 0, client;
    i < this.clients.length;
    i++
  )
    (client = this.clients[i].playerTracker),
      -1 == client.disconnect &&
        ("_socket" in this.clients[i]
          ? client.spectate
            ? spectate++
            : humans++
          : bots++,
